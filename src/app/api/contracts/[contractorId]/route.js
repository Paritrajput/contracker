    ...
    const formattedContracts = await Promise.all(
      blockchainContracts.map(async (c) => {
        const matchingMongoContract = mongoContracts.find((m) => {
          return String(m.winner) === String(c.contractorMongoId);
        });

        if (!matchingMongoContract) {
          console.warn(
            `No matching contract found for blockchain contractId: ${c.contractId}`
          );
          return null;
        }
        ...
        return {
          ...
          mileStones: matchingMongoContract.mileStones,
